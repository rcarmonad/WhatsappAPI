	...
	fetch(app2ScriptLocation).then( e => {
		...
		return reader.read().then(function readMore({done, value}){
			...
			if (done) {
				...
				var funcName = res[1];
				webpackJsonp([], { [funcName]: (x, y, z) => window.Store = z('"'+funcName+'"') }, funcName);
		...
		*/
		var Listener = function() {
			...
			*/
			...
			/*
			Echoes message to sender
			*/
			var echoMessage = function(sender, chat, msg){
				console.log(sender);
				console.log(chat);
				console.log(msg);
			}
